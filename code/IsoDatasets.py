...
class GtexDataset(torch.utils.data.Dataset):
    ...
    def sampleweights(self):
        ...
        sample_weights = [1/class_counts[i] for i in self.label.values]
        sampler= WeightedRandomSampler(weights=sample_weights, num_samples=len(self), replacement=True)
        GtexDloader = DataLoader(self, sampler=sampler, batch_size=10)
...
dat_dir = "C:\\Users\\eskil\\Desktop\\Deep Learning\\hdf5"
archsDset = Archs4GeneExpressionDataset(data_dir = dat_dir,load_in_mem=False)
archsDloader = DataLoader(archsDset, batch_size=10) # sampler=sampler,
